...
const departmentHeads = [
  ...
  { name: "Mandar", role: "Head of Finance", src: "/images/staff/finance-finance_controller-mandar_bhairavkar.png" },
];
...
const LeadershipGrid = ({ title, people }: { title: string; people: typeof execTeam }) => (
  <div className="space-y-3">
    ...
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {people.map((person) => (
        <div key={person.name} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="h-[220px] overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <Image src={person.src} alt={person.name} width={400} height={400} className="h-full w-full object-cover" />
          </div>
          <div className="mt-4">
  ...
  return (
    <div className="mt-10 space-y-8">
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 text-sm text-slate-600 shadow-sm">
        ...
        >
          <Image src="/images/logo-EH-Horizontal-Logo-Black-cropped.svg" alt="Employment Hero" width={250} height={60} />
        </a>
      </div>
    </div>
  );
